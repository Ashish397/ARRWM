#!/bin/bash
# Watch the froz SMOKE (5302281, decay_end=20 -> exercises the freeze in 40
# steps) as a gate for the froz FULL run (5302282, 8 nodes, decay_end=50).
...
SMOKE=5302281; SBASE=smoke-dmd3-fwd-Freal
FULL=5302282;  FBASE=af-stat-wave-dmd3-fwd-Freal
cd /scratch/u6ex/as1748.u6ex/ARRWM
STATUS=/tmp/freal_status.txt
: > "$STATUS"
SSEEN=0; FSEEN=0
...
while true; do
  ...
  [ -n "$problem" ] && { echo "FREAL_PROBLEM $problem" >> "$STATUS"; break; }
  if [ "$SST" = GONE ] && [ "$SSEEN" -eq 1 ] && [ "${SDN:-0}" -gt 0 ] && [ "$(grep -c SMOKE_GREEN "$STATUS")" -eq 0 ]; then
    echo "FREAL_SMOKE_GREEN (smoke complete @40, no errors)" >> "$STATUS"
  fi
  [ "$alldone" -eq 1 ] && { echo "FREAL_ALL_DONE (smoke+full terminal)" >> "$STATUS"; break; }
  [ "$EL" -ge "$HEARTBEAT" ] && { echo "FREAL_HEARTBEAT el=${EL}s" >> "$STATUS"; break; }
  sleep 120
done
