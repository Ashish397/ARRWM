...
for i in $(seq 1 180); do
  ...
  if [ -z "$st" ]; then fin=$(sacct -j $J -n -o State 2>/dev/null|head -1|tr -d ' '); echo "left queue: $fin"; echo "$fin"|grep -qE "FAILED|TIMEOUT|NODE_FAIL|CANCELLED" && { report; tail -8 "$F" 2>/dev/null; exit 1; }; report; exit 0; fi
  ...
  if [ "${odeload:-0}" -ge 1 ] 2>/dev/null && [ "${step:-0}" -ge 1 ] 2>/dev/null; then report; echo "TRAD BASELINE LOADED ODE-800 + REACHED TRAINING (step=$step):"; grep -hE "Loading generator from|v14_teacher|step[ =][0-9]+" "$F" 2>/dev/null | tail -3; exit 0; fi
  echo "[t+$((($(date +%s)-start)/60))m] state=${st} odeload=${odeload:-0} step=${step:-_}"
  sleep 60
done
