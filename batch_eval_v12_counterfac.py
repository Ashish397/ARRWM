#!/usr/bin/env python3
"""Counterfactual batch eval: same as batch_eval_v12.py but with flipped noisy actions.

Noisy-side actions are counterfactual:
  cf_z7 = 1 - current_z7  (invert turn)
  cf_z2 = -current_z2     (reverse forward/back)
Clean-side actions remain unchanged (real ride actions).

Same data order as the normal run for paired comparison.
Output: frodobots_lmdb_counterfac/ and frodobots_noise_counterfac/
"""
...
NOISE_ROOT = "/projects/u6ex/fbots/frodobots_noise_counterfac"
LMDB_ROOT = "/projects/u6ex/fbots/frodobots_lmdb_counterfac"
...
def save_noise_zarr(out_path, latents_list, source_attrs, city):
    ...
    for k, v in source_attrs.items():
        g.attrs[k] = v
    g.attrs["generated_by"] = "v12_batch_eval_counterfactual"
    g.attrs["city"] = city
    ...
    for ride_i, (manifest_idx, ts, city, ride) in enumerate(my_rides):
        ...
        try:
            ...
            while offset + STREAM_LATENT_SPAN <= n_lat:
                ...
                # Counterfactual: flip noisy-side actions
                # action_dims = [2, 7] → index 0 = z2 (forward/back), index 1 = z7 (turn)
                z_noisy_cf = z_noisy.clone()
                z_noisy_cf[..., 1] = 1.0 - z_noisy[..., 1]  # cf z7 = 1 - current z7
                z_noisy_cf[..., 0] = -z_noisy[..., 0]        # cf z2 = -current z2
                z_noisy = z_noisy_cf

                # Build conditional (same as eval_chain, but with counterfactual noisy actions)
