"""EVALUATED AND REJECTED — retained for provenance, used for no reported number.

Depth-field curvature as a geometric-corruption signal (Depth-Anything-V2).
Reported in the paper's instrument-validation table at AUC 0.52, i.e. near
chance, and excluded from the final ensemble.

Requires a `fleet` harness module that is not part of this release, so it will
not run as shipped. It is included because the reproducibility checklist
undertakes to ship the code behind rejected as well as adopted instruments.
"""
...
from analysis.testbench_v2.geometry_metrics import DepthField
