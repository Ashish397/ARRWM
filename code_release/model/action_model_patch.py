...
adaLN-Zero modulation. The only behaviour change is:

  • add `_action_modulation` (if provided) to the output of `time_projection`
  • thread `_action_modulation` through `_forward_inference` and
    `_forward_train`

Everything else stays untouched, so with no action supplied the model is
...
def patch_causal_wan_model_for_action(model):
    ...
    # Thread modulation through _forward_inference.
    ...
    # Thread modulation through _forward_train.
