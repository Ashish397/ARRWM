...
class VideoLoader:
    ...
    def find_actual_video_path(self, json_path: str) -> str:
        ...
        base_path = Path(os.environ.get("DATA_ROOT", "data")) / "frodobots" / "train"
...
def load_longlive_vae(device='cuda', dtype=torch.float16):
    ...
    import sys
    sys.path.append(os.environ.get('LONGLIVE_ROOT', 'third_party/LongLive'))
...
def main():
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=os.path.join(os.environ.get("DATA_ROOT", "data"), "frodobots", "train"),
        help="Path to FrodoBots dataset"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=os.path.join(os.environ.get("DATA_ROOT", "data"), "frodobots_encoded", "train"),
        help="Path to save encoded videos"
    )
    parser.add_argument(
        "--vae_path",
        type=str,
        default=os.path.join(os.environ.get("WAN_MODELS", "wan_models"), "Wan2.1-T2V-1.3B"),
