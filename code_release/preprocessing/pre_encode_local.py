...
# Data locations come from the environment so this script carries no
# machine-specific paths. DATA_ROOT holds the FrodoBots corpus;
# WAN_MODELS holds the pretrained Wan checkpoints.
_DATA_ROOT = os.environ.get("DATA_ROOT", "data")
_WAN_MODELS = os.environ.get("WAN_MODELS", "wan_models")
...
def main() -> None:
    ...
    p.add_argument("--zarr_root", default=os.path.join(_DATA_ROOT, "frodobots_dataset"),
                   help="7K zarr root")
    p.add_argument("--r2k_base", default=_DATA_ROOT,
                   help="Base for relative 2K paths")
    p.add_argument("--output_root", default=os.path.join(_DATA_ROOT, "frodobots_encoded"),
                   help="Output dir; writes <root>/<ride_ts>.zarr/")
    p.add_argument("--overwrite", action="store_true", help="Overwrite existing zarr")
    p.add_argument("--vae_path",
                   default=os.path.join(_WAN_MODELS, "Wan2.1-T2V-1.3B", "Wan2.1_VAE.pth"))
