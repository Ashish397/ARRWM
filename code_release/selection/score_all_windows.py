"""Traverse the ride manifest and score every block-aligned 24-latent
...
This is the data-prep traversal: later we select the top-X% by
...
# Run from the release root so the relative asset paths below resolve.
_ROOT = os.environ.get("AF_ROOT") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
ds = ZarrRideDataset.from_manifest(rides_data=rides, motion_root=args.motion_root,
                                   pca_basis_checkpoint=args.pca_basis, device=dev)
N = len(ds)
