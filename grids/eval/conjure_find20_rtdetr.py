"""Conjuration find-20 with a DIFFERENT detector: RT-DETR (transformer, set-prediction) instead
of Faster R-CNN. No sharpness gate. Same probe: track-birth of a NEW object in the central band
(not entering from a side edge) into a spot that was empty in the recent past. Transformer
detectors emit a fixed small set of object queries, so they hallucinate far less inside melt/warp
than a dense-anchor CNN. Runs shuffled fleet videos until 20 conjured objects, draws each box."""
import os
...
OUT = os.path.join(OUTD, "conjure_find20_rtdetr.png")
MODEL_ID = "PekingU/rtdetr_r50vd"
KEEPNAMES = {"person", "bicycle", "car", "motorcycle", "bus", "truck", "backpack", "handbag", "suitcase"}
SCORE = 0.80; CX = (0.20, 0.80); EDGE = 0.12; MIN_AREA = 0.008
...
def main():
    ...
    def detect(frame):
        H, W = frame.shape[:2]
        inp = proc(images=frame, return_tensors="pt").to(DEV)
        out = model(**inp)
        res = proc.post_process_object_detection(out, target_sizes=[(H, W)], threshold=SCORE)[0]
        dets = []
        for b, l, s in zip(res["boxes"].cpu().numpy(), res["labels"].cpu().numpy(), res["scores"].cpu().numpy()):
            if id2label[int(l)] in KEEPNAMES:
                dets.append((b, id2label[int(l)]))
        return dets
    ...
    def events_for(scene, mdl):
        ...
        for k, fr in enumerate(frames):
            ...
            for di, (b, l) in enumerate(ds):
                ...
                tracks.append({"box": b, "cls": l, "birth_k": k, "last": k, "cen": ((b[0]+b[2])/2, (b[1]+b[3])/2), "central": central})
        ...
        for t in tracks:
            if t["birth_k"] > 0 and t["central"] and t["last"]-t["birth_k"] >= 1 and empty(t):
                out.append(dict(scene=scene, mdl=mdl, cls=t["cls"], img=frames[t["birth_k"]].copy(), box=t["box"].astype(int)))
        return out
    ...
    for e in events:
        ...
        cv2.putText(h, f"{e['mdl']}_{e['scene']} {e['cls']}", (3, 13), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (230, 230, 230), 1)
    ...
    rows = [np.concatenate(cells[i:i+5] + [np.full((ch, cw, 3), 25, np.uint8)]*(5-len(cells[i:i+5])), 1) for i in range(0, len(cells), 5)]
