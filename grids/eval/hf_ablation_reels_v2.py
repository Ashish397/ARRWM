"""[HF-Cal-v2 version] HF-degradation (Laplacian-variance drift B) validation reel on OUR ablation variants only
...
def main():
    ...
    # HF-Cal-v2: replace the v1 sibling-relative B with the reference-relative B_v2 (six externals + the variant itself)
    F = pd.read_csv(os.path.join(HERE, "out", "final_flags_v2.csv"))[["scene", "model", "B_v2"]]
    d = d.merge(F, left_on=["grid", "model"], right_on=["scene", "model"], how="left")
    assert d.B_v2.notna().all(), int(d.B_v2.isna().sum())
    d["B"] = d["B_v2"]
