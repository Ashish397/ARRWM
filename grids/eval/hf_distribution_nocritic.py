"""Figure 26 (ablation HF-degradation ridgeline) regenerated WITH the No Critic
variant. Identical mask (feature_valid AND active), identical B definition,
identical axes and B=150 reference line as hf_distribution.py; No Critic's B
values and mask come from nocritic_final_eval.py, which reproduces the released
instrument conventions exactly.
"""
import os
...
REF = f"{ARR}/code_release/evaluation/quality/reference"
OUT = os.path.join(HERE, "out", "hf_distribution_nocritic.png")
OURS = {"pca8", "pca4", "pca2", "16node", "4node", "noatok", "noadaln"}

mask = pd.read_csv(f"{REF}/canonical_static_mask.csv")
...
hf = pd.read_csv(f"{REF}/fleet_hf.csv")
d = keep.merge(hf[["model", "scene", "B"]], on=["model", "scene"])

# --- append No Critic (same mask + same B definition) ---
nc = pd.read_csv(os.path.join(HERE, "out", "nocritic_final_eval.csv"))
nc = nc[(nc.feature_valid) & (nc.active == 1)][["scene", "B"]]
nc["model"] = "ours_nocritic"
d = pd.concat([d, nc[["model", "scene", "B"]]], ignore_index=True)
...
for i, v in enumerate(order):
    ...
    col = "#4C72B0" if v == "ours_nocritic" else "#55A868"
...
print("\nmedian B and B>150 rate on the active population:")
for v in order:
    b = d[d.model == v].B.values
    print(f"  {DISPLAY[v]:18s} n={len(b):3d}  median B {np.median(b):+7.1f}   B>150 {int((b>150).sum()):3d} = {100*(b>150).mean():5.1f}%")
