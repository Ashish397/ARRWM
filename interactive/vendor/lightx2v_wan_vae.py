# Vendored from LightX2V (ModelTC/LightX2V), file
#   lightx2v/models/video_encoders/hf/wan/vae.py
# fetched 2026-08-31 from https://github.com/ModelTC/LightX2V (Apache-2.0).
# Original header: "Copyright 2024-2025 The Alibaba Wan Team Authors. All rights reserved."
#
# Trimmed for the interactive world model: only the nn.Module stack that the
# `lightvaew2_1` checkpoint needs is kept (CausalConv3d .. WanVAE_).  The
# LightX2V `WanVAE` outer wrapper (tiling / torch.distributed sequence-parallel
# / cpu-offload paths) and the lightx2v-internal imports
# (`load_weights`, `GET_USE_CHANNELS_LAST_3D`, `AI_DEVICE`) are removed;
# nothing else in the retained code was modified.
#
# NOTE: this is NOT the same architecture as the repo's own wan/modules/vae.py.
# LightX2V's Decoder3d halves `in_dim` at upsample stages 1/2/3 and the
# checkpoint is a `pruning_rate=0.75`, `dim=96` distillation, so the Wan
# reference module cannot load these weights.
