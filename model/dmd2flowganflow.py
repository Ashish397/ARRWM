...
class DMD2(SelfForcingModel):
    ...
    def ensure_flow_discriminator_initialized(self, image_or_video_shape) -> None:
        """
        Ensure the flow discriminator exists before optimizers are constructed.
        """
        if self.flow_discriminator is not None:
            return
    ...
    def generator_loss(
        ...
        # Real-data flow alignment term for domain shift / action-conditioned student; random timestep for efficiency
        if self.generator_real_flow_loss_weight > 0.0 and real_latents is not None:
            batch_size, num_frame = pred_image.shape[:2]
            real_latents_aligned = real_latents.to(device=pred_image.device, dtype=torch.float32)
            
            # Align number of frames if the targets include more temporal steps than the rollout
            min_frames = min(pred_image.shape[1], real_latents_aligned.shape[1])
            real_latents_aligned = real_latents_aligned[:, :min_frames]
            
            assert real_latents_aligned.shape[0] == batch_size, "real_latents batch must match pred_image batch"
            
            # Sample a random timestep (per batch element), expand to [B, F]
            # Always use random sampling (not _get_timestep) for speed
            # Use the same min/max timestep bounds as the FlowGAN GAN timestep logic
            min_timestep = denoised_timestep_to if self.ts_schedule and denoised_timestep_to is not None else self.min_score_timestep
            max_timestep = denoised_timestep_from if self.ts_schedule_max and denoised_timestep_from is not None else self.num_train_timestep
            t = torch.randint(min_timestep, max_timestep, (batch_size,), device=pred_image.device, dtype=torch.long)
            real_flow_timestep = t[:, None].repeat(1, min_frames)
            
            if self.timestep_shift > 1:
                real_flow_timestep = self.timestep_shift * \
                    (real_flow_timestep / 1000) / (1 + (self.timestep_shift - 1) * (real_flow_timestep / 1000)) * 1000
            real_flow_timestep = real_flow_timestep.clamp(self.min_step, self.max_step)
            
            # Create x_t from REAL latents (real_latents as x0) using scheduler.add_noise
            real_flow_noise = torch.randn_like(real_latents_aligned)
            real_x_t = self.scheduler.add_noise(
                real_latents_aligned.flatten(0, 1),
                real_flow_noise.flatten(0, 1),
                real_flow_timestep.flatten(0, 1)
            ).unflatten(0, (batch_size, min_frames))
            real_x_t = real_x_t.detach()
            
            # Compute flow_target for REAL data using the SAME conversion used in critic_loss
            # IMPORTANT: compute flow_target under torch.no_grad() so it is treated as a fixed target
            with torch.no_grad():
                flow_target = WanDiffusionWrapper._convert_x0_to_flow_pred(
                    scheduler=self.scheduler,
                    x0_pred=real_latents_aligned.flatten(0, 1),
                    xt=real_x_t.flatten(0, 1),
                    timestep=real_flow_timestep.flatten(0, 1)
                )
                flow_target = flow_target.unflatten(0, (batch_size, min_frames))
            
            # Compute student's flow prediction at that same (x_t, t)
            # Forward the student generator once at (x_t, t, conditional_dict) to get pred_x0_gen
            out = self.generator(
                noisy_image_or_video=real_x_t,
                conditional_dict=conditional_dict,
                timestep=real_flow_timestep
            )
            ...
            # Convert pred_x0_gen -> flow_pred_gen with WanDiffusionWrapper._convert_x0_to_flow_pred
            ...
            flow_pred_gen = flow_pred_gen.unflatten(0, (batch_size, min_frames))
            
            # Compute supervised real-data flow loss
            real_flow_loss = F.mse_loss(flow_pred_gen.float(), flow_target.float())
            weighted_real_flow_loss = real_flow_loss * self.generator_real_flow_loss_weight
            total_loss = total_loss + weighted_real_flow_loss
            
            # Log both weighted and raw loss values
            dmd_log_dict.update({
                "generator_real_flow_loss": weighted_real_flow_loss.detach(),
                "generator_real_flow_raw": real_flow_loss.detach(),
                "real_flow_timestep_mean": real_flow_timestep.float().mean().detach(),
            })
