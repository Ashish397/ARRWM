...
#!/bin/bash
# =====================================================================
# DMD3 (2026-08-18) — the recipe that works, after a leave-one-out sweep.
#
# LOO RESULTS (all 91 steps, MSE init, holders 6044047/6044048, baseline =
# lowtfix2SA):
#   -stat anchor      -> DIES (~61; lowtfix2). LOAD-BEARING.
#   -band-local norm  -> indistinguishable. INERT.
#   -critic shift     -> indistinguishable. INERT.
#   +discrete rungs   -> CLEARLY BETTER. Adopted here.
#
# HONEST NOTE ON THE MECHANISM: an earlier write-up in train_dmd10k_lowt/
# _trad claimed the failure was a blurry critic (trained at the high-t
# scoring shift 5.0) turning pred_fake - pred_real into a high-frequency
# pump. That story predicted `-critic shift` would degrade. It did NOT.
# The mechanism is therefore UNSUPPORTED. `aux_teacher_timestep_shift=1.0`
# is kept only because it matches the June trad run, not because it was
# shown to do anything here. Same for the band-local normalizer.
#
# `dmd_sample_at_rungs=true` draws the DMD scoring t from the student's OWN
# ladder [1000, 625, 357.142857, 208.333333] instead of a shifted continuum
# (model/dmd_action_forcing.py, `_sample_dmd_timestep`). Rationale: the
# 4-rung student only ever OCCUPIES those noise levels, so sampling between
# them asks the teacher about states the student never visits.
# IMPORTANT: the rung sampler RETURNS EARLY, before the shift is applied, so
# `timestep_shift=0.5` and `min_score_timestep=150` below are now INERT.
# They are left in place only so this file diffs cleanly against
# train_dmd10k_lowt.sbatch; do not read them as active.
#
# STILL UNTESTED — the geometry. Every arm so far runs 42f with
# clean_match/clean_drift OFF, i.e. clean_x = PAST GT holding the exact GT
# counterpart of every supervised student frame at the same rotary index.
# June ran either the self-view (trad, dmd_42f_enabled=false) or FUTURE GT
# (clean_match_forward / clean_drift). Ours is the one configuration June
# never used. The G1-G4 block tests this.
# =====================================================================
...
: "${AR_HEAD:?AR arm requires AR_HEAD=1.0 (else it is a silent TF duplicate)}"
...
srun --jobid=$HOLDER --overlap --nodelist=$NODES --nodes=$NNODE --ntasks-per-node=1 --gpus-per-node=4 --gpu-bind=none --export=ALL,CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun \
  --nnodes=$NNODE \
  --nproc_per_node=4 \
  --rdzv_id=ar${DARM}$HOLDER$PORTOFF \
  --rdzv_backend=c10d \
