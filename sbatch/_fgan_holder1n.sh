...
mkdir -p "$LOGDIR" logs
MASTER_ADDR=nid010188
MASTER_PORT=$((29500 + HOLDER % 16000 + PORTOFF))
...
srun --jobid=$HOLDER --overlap --nodelist=nid010188 --nodes=1 --ntasks-per-node=1 --gpus-per-node=4 --gpu-bind=none --export=ALL,CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun \
  --nnodes=1 \
  --nproc_per_node=4 \
  --rdzv_id=fg${DARM}$HOLDER \
  --rdzv_backend=c10d \
