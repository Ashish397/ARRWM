#!/bin/bash
# 8-NODE REAL 13B-untrained arm (promotion clone of 13b v3 — carries the
# PROVEN memory recipe: max_real=4, disc micro-batch 8; step-73 wall
# cleared at 89.1GB peak on 2 nodes, per-rank footprint identical at 8).
# smoke_13bdisc v3 (MAIN 11:5x) — v2 died DETERMINISTICALLY at ~step 73 on
# all four instances: same seed -> same big-ride (4-chunk, depth-6) arrives
# at the same step; steady GAN peak 91.5 GB leaves < 1.1 GiB for its gen
# transient. FT_v3-class relief, ~2-4 GB of peak:
#   ladd_gt_transition_match_max_real 8 -> 4   (halves D-batch real rows)
#   ladd_disc_micro_batch_groups     4 -> 8   (finer disc backward)
# max_real=4 is an EXPERIMENT-VISIBLE change — flagged in grid + to
# researcher; alternative was no 13B run at all.
...
export DARM=arm13b8n
export MAXSTEPS=${MAXSTEPS:-500}
