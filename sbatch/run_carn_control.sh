#!/bin/bash
# CARN PAIR-SWAP TEST — CONTROL (researcher directive 2026-08-24).
# ENTIRE CARN functionally disabled: FN loss weight 0.0, no application,
# seam 0.0. FN module IS constructed (inert) so its weight-init consumes
# the SAME global-RNG draws as the arm (else streams desync at construction
# and every ride/depth/noise draw differs: observed 9s vs 6s videos).
# Pixel GAN OFF. Otherwise byte-identical DEXTRA to run_carntx_arm.sh.
