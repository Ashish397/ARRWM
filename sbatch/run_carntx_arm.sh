#!/bin/bash
# CARN PAIR-SWAP TEST — ARM (researcher directive 2026-08-24).
# fn_pair_mode=rollout_to_gt: the forward noiser trains F(rollout,lvl)~=GT
# at the same ride/abs positions — F IS the style corrector (rollout->GT).
# No rollout-2 prebuild, no reverse net, no cycle. Application is OFF:
# this run only TRAINS the corrector; verdict comes offline (apply F to
# C_late latents -> decode -> texture_battery vs B) + arm-vs-control eval.
# Pixel GAN OFF. Seam affine 0.0 — matched to control (researcher: the pair
# differs ONLY in the new TX corrector; the old seam affine is not the TX).
# Watch stderr for: [FN-TRAIN][fn_pair_mode=rollout_to_gt] corrector ALIVE
...
export PORTOFF=${PORTOFF:-9971}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=carntx_arm
export MAXSTEPS=${MAXSTEPS:-120}
