#!/bin/bash
# CARN REVERSE TEST — ARM (researcher, 2026-08-24). EXACT ganv2f_all_fnoiser
# recipe (GAN influence channel intact) with ONLY the pair swap, via the
# EXISTING h1 flag: forward_noiser_reverse=true + carn_recurse=false +
# forward_noiser_chain_levels=false => FN(student chunk @L) -> clean GT
# (input = rollout chunk, target = GT). Applied to GT formers it de-CARNs
# the real distribution the disc sees; generator influenced through the GAN
# exactly as in dmd10k_ganv2f_all_fnoiser_h6107082_135210.
...
export DARM=carntx_fakefeat
export MAXSTEPS=${MAXSTEPS:-150}
export CKPT_EVERY=25
