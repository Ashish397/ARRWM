#!/bin/bash
# decouple13b — GAN_REDESIGN_TWO Option B REAL ARM: 300 steps, disc on the
# STOCK Wan-1.3B prefix (decoupled; RESUME TRAP: must be fresh-disc —
# auto_resume=false, verify in startup log). noanchor base, stat anchor
# OFF, TF head, A-features on. Weight 0.03 initial; read the unweighted
# division at ~step 50 per protocol.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9991}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=decouple13b
