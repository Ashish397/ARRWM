#!/bin/bash
# decouple14b — GAN_REDESIGN_TWO Option A REAL ARM: 300 steps, LADD disc
# projected on the Wan-14B prefix (taps [0,2,4,8]), WP-14B's measured
# memory fix (G groups=16, max_real=8), noanchor base (STAT ANCHOR OFF),
# TF head. Weight starts at GANW-equivalent 0.03; A7 gan_dmd_grad_ratio is
# read at step ~50 per the probe protocol — if <<5% band, relaunch scaled.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9988}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=decouple14b
export MAXSTEPS=${MAXSTEPS:-300}
