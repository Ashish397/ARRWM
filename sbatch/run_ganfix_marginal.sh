#!/bin/bash
# MARGINAL real-vs-fake arm: the primal texture question, no transition frame.
# Identical to run_ganfix_strict_rerun.sh except the pair construction:
#   * gt_transition OFF; gt_vs_fake ON (single-chunk "is this real or
#     generated?", the LADD-canonical mode — texture is a marginal,
#     stationary property, and the 2-chunk transition frame lets the critic
#     spend capacity on seam cues instead)
#   * all_pairs=true + n_real=16: every one of 16 FRESH GT chunks (resampled
#     from the full ride window each D-update) scored against every student
#     chunk — an unpaired, diverse real batch, not nearest-matched
#   * match=false (nearest-match machinery is mutually exclusive with the
#     all-pairs/n_real branch — the trainer raises if both are set)
# Cross-ride ring flags kept but INERT here (the ring stores 2-chunk
# transition slabs; the gt_vs_fake path never samples it — telemetry will
# show train/ladd_ring_on=0). Everything DMD-side matches the strict rerun.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9972}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganfix_marginal
