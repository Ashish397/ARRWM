#!/bin/bash
# B9 POOL-DIVERSITY arm ("the disc isn't seeing enough real-world data").
# Identical to run_ganfix_strict_rerun.sh except the REAL-side supply:
#   * match_k 4 -> 8            (twice the reals per fake)
#   * match_pool 8 -> 100000    (=> clamped to the WHOLE ride window: local
#                                picks become uniform-random over every GT
#                                transition in the ride instead of the top-8
#                                L1-NEAREST — the nearest-match rule feeds the
#                                disc exactly the reals most similar to the
#                                student's output, the least informative
#                                choice for a texture critic)
#   * match_max_real 12 -> 16   (distinct-reals cap per update; halved to 8
#                                on the matched half when the ring is on)
#   * cross-ride ring 4096 -> 8192, push_per_ride 8 -> 32 (ring fills ~4x
#                                faster; with match_k=8 the ring serves 4
#                                slots per fake instead of 2)
#   * ladd_disc_micro_batch_groups=4 (memory guard for the larger real batch)
# Telemetry (gan_grad_telemetry_every, train/ladd_ring_*) confirms the ring
# actually populates and what the GAN gradient does relative to DMD.
set -euo pipefail
