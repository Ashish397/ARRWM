#!/bin/bash
# POOLRICH v2 — researcher-directed sharpness fix (2026-08-23).
# Clone of run_ganfix_poolrich.sh with EXACTLY TWO deltas:
#   1. dmd_loss_start_step 50 -> 0   (researcher: "DMD needs to start from
#      step 0"; v1 ran GAN-only from step 20-50, so early samples were
#      DMD-starved and soft)
#   2. ladd_disc_sample_t true(shift 5.0) -> false (fixed near-clean t0 disc
#      — the raw_t0 cell the researcher judged best; shift 5.0 puts 76% of
#      disc samples above t=625 where texture is destroyed, and R7 already
#      rules it out)
# Everything else (low lr, EMA-from-0, GANW 0.03, 5 disc updates, diverse
# pool, eq off, action-cond) kept from v1 per researcher sign-off.
# Secondary sharpness levers NOT taken (flagged): ema_start_step=0 means the
# step-200 eval ckpt is EMA-smoothed; fake_score_ema_weight=0.0.
# TF head ONLY — NEVER the AR head (standing rule 1).
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9973}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganfix_poolrich_v2
