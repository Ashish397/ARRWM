#!/bin/bash
# ganv2f_all — fix-validation wave 2026-08-23. Base = run_ganfix_poolrich_v2.sh
# (poolrich + dmd_loss_start_step=0 + fixed-t0 disc, researcher-approved).
# ALL implemented fixes on the v2 base: patch logits+R1 token-norm, flip+translation aug, early taps [0,2,4,8,29], tripwire+diversity telemetry.
...
export PORTOFF=${PORTOFF:-9974}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganv2f_all
