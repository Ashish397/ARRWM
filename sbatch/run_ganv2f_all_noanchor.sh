#!/bin/bash
# ganv2f_all_noanchor — researcher-requested 2026-08-23: the ALL-fixes arm
# with the CARN stat anchor DISABLED (STAT_ANCHOR=0.0 -> stat_anchor_loss_weight=0).
# Replaces ganv2f_noaug (killed ~step 65 on researcher instruction).
# Everything else identical to run_ganv2f_all.sh. TF head only (rule 1).
set -euo pipefail
...
export DARM=ganv2f_all_noanchor
