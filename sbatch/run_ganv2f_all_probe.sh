#!/bin/bash
# ganv2f_all_probe — all_noanchor base + A22 held-out probe ENABLED
# (disc_holdout_probe_every=50): live test of the leak guards
# (dhp_leak_rides/dhp_leak_seen must stay 0) inside a real 200-step arm.
# Doubles as a near-replicate of all_noanchor for training-noise estimate.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9982}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganv2f_all_probe
