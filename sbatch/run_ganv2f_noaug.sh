#!/bin/bash
# ganv2f_noaug — fix-validation wave 2026-08-23. Base = run_ganfix_poolrich_v2.sh
# (poolrich + dmd_loss_start_step=0 + fixed-t0 disc, researcher-approved).
# Leave-one-out: ALL minus translation augmentation (flip only).
...
export PORTOFF=${PORTOFF:-9976}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganv2f_noaug
