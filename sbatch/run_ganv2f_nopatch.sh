#!/bin/bash
# ganv2f_nopatch — fix-validation wave 2026-08-23. Base = run_ganfix_poolrich_v2.sh
# (poolrich + dmd_loss_start_step=0 + fixed-t0 disc, researcher-approved).
# Leave-one-out: ALL minus the patch-logit pair (scalar_output stays true, r1_normalize_tokens stays false).
# KNOWN RISK (A16) in patch-logit arms: match-fallback steps route to the
# un-normalised positional R1 (~T^2); match=true makes this rare — watch for
# [LADD-MATCH-FALLBACK] + d_loss spikes. TF head only (standing rule 1).
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9975}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=ganv2f_nopatch
