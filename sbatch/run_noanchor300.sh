#!/bin/bash
# noanchor300 — 300-step control matched to pixtex300 (same base, no pixel
# flags). Extends the noanchor recipe past the 200-step horizon.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9986}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=noanchor300
