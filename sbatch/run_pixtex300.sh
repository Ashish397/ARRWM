#!/bin/bash
# pixtex300 — 300-step pixel-GAN integration arm (T3-A + T1 + A22 probe).
# NOTE: T3-B/C (D-loop + G-term) not landed at composition time; the cmd
# file re-checks at pick-up and aborts with a message if still missing the
# gate the researcher wants tested. Control = noanchor300.
set -euo pipefail
