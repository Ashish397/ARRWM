#!/bin/bash
# smoke_coupled_afeat — researcher-ordered testbed: the OLD coupled
# DMD-teacher critic (known GAN<->DMD compounding mode) with the A19-A24
# machinery ENABLED (holdout probe, unified R1 cadence, diversity telemetry,
# tripwire). 60 steps, stat anchor OFF. Purpose: exercise the A-features.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9989}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=smoke_coupled_afeat
