#!/bin/bash
# smoke_pixgrad — BID PIXGAN-1 (GRANTED 6110540): smoke_pixgan's exact
# config with ONE flag changed: pix_finish_grad_enabled=TRUE. First-ever
# in-vivo execution of the A23 inference-parity grad path (seam bug hid it
# in smoke_pixtex; smoke_pixgan sets it false by design). MECHANICS ONLY.
# Readouts: [A23] one-shots, pix_finish_grad_* counters, 14.3% dilution
# check, ladder-fake weight-probe point, KV recompute-order hazard.
set -euo pipefail
