#!/bin/bash
# smoke_pixtex — HISTORICAL (ran 2026-08-23 17:50, 60/60 PASS, certified
# T3-A + probe integration). Gate flipped OFF post-hoc: re-running with the
# gate on would now boot the pre-D2 pixel arm (T3-C landed underneath it) —
# queue-time-vs-start-time skew. For a live pixel arm, write a NEW script
# from the WP-PIXGAN handoff config, never re-run this one.
set -euo pipefail

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9984}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=smoke_pixtex
