#!/bin/bash
# smoke_surrogate — MECHANICS smoke of the B3 surrogate path (wired by MAIN
# 2026-08-24, docs/TASK_SURROGATE_CONSUMPTION.md; 30 wiring tests green).
# = the pixgan mechanics smoke with surrogate_critic_enabled=true flipped:
# pixel D-loop trains as before (it is the surrogate's teacher), the
# distiller fits the latent critic each step (refresh cadence default 4),
# and the generator's texture gradient comes from the LATENT critic (the
# consumption branch) instead of the decode->disc direct path. w=0.01
# mechanics-only, pix_r1_gamma=0.0 explicit R1-off, stat anchor OFF.
# Reads: train/surrogate_* (consumed=1, n_teacher_refresh cadence,
# grad_cos_sim, critic_disc_corr), plus the pixel D-side keys unchanged.

cd /scratch/u6ex/as1748.u6ex/ARRWM
: "${HOLDER:?set HOLDER to a running holder job ID}"

export PORTOFF=${PORTOFF:-9992}
export RUNSTAMP=${RUNSTAMP:-$(date +%H%M%S)}
export DARM=smoke_surrogate
