...
Signal: ss_vae z7 (throttle), sign convention + = forward, - = reverse.
...
ds = ZarrRideDataset.from_manifest(rides_data=rides, motion_root=args.motion_root,
                                   ss_vae_checkpoint=args.ss_vae, device='cpu', ss_vae_device='cpu')
N = len(ds)
...
for i in range(N):
    try:
        ...
        z2 = z[:, 2].float().numpy()   # steering / yaw (turning)
        z7 = z[:, 7].float().numpy()   # throttle (+fwd / -rev)
        all_z7.append(z7)
        ok += 1
