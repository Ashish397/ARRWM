"""Mangle iteration 4 (r08 BL): STRUCTURED novelty + temporal warp-residual.
...
Saves analysis/eval_final/mangle_iter4_r08.csv
"""
import os
...
def main():
    ...
    for run in RUNS:
        ...
        row = dict(run=run)
        for m in models:
            bank = torch.cat([feats(unfold(t(i)), m) for i in SEED_IDX], 0)
            scores = []
            for i in GEN_IDX:
                ps = unfold(t(i))
                e = feats(ps, m)
                ...
                w = grad_weight(ps)
                scores.append(float(np.percentile(d * w, 95)))
            row[f"snov_{m}_max"] = round(max(scores), 4)
            row[f"snov_{m}_mean"] = round(float(np.mean(scores)), 4)
        wr = []
    ...
    df = pd.DataFrame(rows); df.to_csv("analysis/eval_final/mangle_iter4_r08.csv", index=False)
