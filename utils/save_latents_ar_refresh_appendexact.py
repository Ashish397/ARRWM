#!/usr/bin/env python3
"""AR_refresh path emulating append_baseline as exactly as possible.

Three changes from vanilla AR_refresh, all combined to give append-baseline
the closest matching K/V semantics inside the AR_refresh path:

1. **Global RoPE indices** (monkey-patch ``rope_apply`` to add a per-step
   global offset = ``ctx_global_lo``).
2. **Post-RoPE K and V cached** (monkey-patch ``flex_attention`` to
   override ``key`` and ``value`` at ctx token positions before
   attention runs).
3. **Capture point = commit refresh forward** — at the end of each
   step, run an extra forward with the just-committed clean chunk in
   the live slot, ctx K/V replayed from cache, and capture chunk_c's
   K and V. This matches append_baseline's
   ``cache_refresh="append"`` mode where the persistent cache is
   refreshed at t=0 with the clean pred_x0 right after the 4 denoise
   passes finish.

If this *still* doesn't reproduce the |0.3| spectral peak, then the
peak is not driven by stale ctx K/V values — it must come from the
fact that ``_forward_inference`` only runs the live chunk through the
model (ctx tokens are pure cache reads) while AR_refresh's
``_forward_train`` propagates ctx tokens through every transformer
block. That structural difference (ctx tokens having internal hidden
state evolution within the joint forward) would then be the smoothing
factor.
"""
    ...
    def patched_flex(query, key, value, block_mask=None, **kwargs):
        ...
        if state.chunk_position_map:
            for chunk_id, lo, hi in state.chunk_position_map:
                # Skip the chunk we're currently capturing or the live chunk.
                if chunk_id == state.current_chunk_id and state.capture_chunk_id != chunk_id:
                    continue
                if chunk_id == state.capture_chunk_id:
                    continue
                slot = state.cache.get(chunk_id, {}).get(layer_idx)
                if slot is None:
                    continue
                k_cached = slot.get("k")
                v_cached = slot.get("v")
...
def run(
    ...
    log.info("AR_refresh appendexact rollout: seed=%d  gen=%d  fifo=%d (global RoPE + commit-refresh capture + post-RoPE K+V cache)",
    ...
    try:
        for chunk_idx in range(int(num_gen_chunks)):
            ...
            # Per-step global RoPE offset = window's first frame's global index.
            ...
            # 4 denoise passes — replay ctx K/V; live K/V fresh.
            ...
            # COMMIT REFRESH FORWARD: chunk_c at t=0 in live slot, capture
            # K and V (post-RoPE) for chunk_c. Older ctx K/V replayed from
            # cache. This matches append_baseline's
            # cache_refresh='append' commit refresh.
...
def main():
    ...
    log.info("ar_refresh_appendexact latents=%s wall=%.1fs", tuple(lat_cpu.shape), wall)
    ...
    torch.save({
        "meta": {
            ...
            "variant": "ar_refresh_appendexact",
            "note": "AR_refresh + global RoPE + commit-refresh capture + post-RoPE K+V cache",
        },
        "gt": gt_latents,
        "ar_refresh_appendexact": lat_cpu,
    }, out / "latents.pt")
    log.info("saved %s", out / "latents.pt")

    lat_dev = lat_cpu.to(device=device, dtype=dtype)
    video_np = pipe.decode_latents(lat_dev)
    frames_to_mp4(video_np, str(out / "ar_refresh_appendexact.mp4"), fps=args.video_fps)
    log.info("wrote %s", out / "ar_refresh_appendexact.mp4")
    ...
    with (out / "manifest.json").open("w") as fh:
        json.dump({"variant": "ar_refresh_appendexact", "wall_s": wall, "status": "ok"}, fh, indent=2)
