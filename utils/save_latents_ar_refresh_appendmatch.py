#!/usr/bin/env python3
"""AR_refresh path emulating append_baseline's full caching semantics.

Two changes from vanilla AR_refresh:

1. **RoPE uses global frame indices** (matches append's
   ``current_start_frame`` convention) instead of window-relative
   indices. The entire window's tokens get ``temporal_offset =
   ctx_global_lo``. So a chunk's frame indices in the RoPE table are
   ``[chunk_global_frame_0, chunk_global_frame_1, chunk_global_frame_2]``
   regardless of which slot of the window the chunk occupies in any
   given step. Q for new tokens also uses their global indices, so
   relative offsets between Q and K stay consistent (this is what
   makes append's stale K not produce color-noise — the rotation is
   "correct in absolute terms" even if the K content is stale).

2. **Write-once post-RoPE K+V cache for ctx chunks**. On first
   appearance of a chunk's tokens during a forward, the post-RoPE K
   and the V tensor are captured into the cache. On all subsequent
   forwards, the cached values overwrite K and V at those token
   positions before flex_attention runs. Cache entries are never
   updated. FIFO eviction.

Together these two pieces give the AR_refresh path the same K/V-cache
semantics as append_baseline. If the resulting video stutters /
staggers like append_baseline does, the failure mode is reproduced
inside AR_refresh — confirming that what AR_refresh's per-pass refresh
actually buys is breaking out of stale ctx K/V (not anything about
window-relative RoPE).

Saves latents.pt + ar_refresh_appendmatch.mp4 + gt.mp4.
"""
...
def install_patches(state: _State):
    orig_rope = cm.rope_apply
    orig_flex = cm.flex_attention

    def patched_rope(x, grid_sizes, freqs, temporal_offset=0):
        # Treat the call's ``temporal_offset`` as additive to the global
        # offset we set per rollout step.
        return orig_rope(x, grid_sizes, freqs,
                         temporal_offset=state.global_temporal_offset + temporal_offset)

    def patched_flex(query, key, value, block_mask=None, **kwargs):
        ...
        if state.chunk_position_map:
            for chunk_id, lo, hi in state.chunk_position_map:
                if chunk_id == state.current_chunk_id:
                    continue
                slot = state.cache.setdefault(chunk_id, {}).setdefault(layer_idx, {})
                k_cached = slot.get("k")
                v_cached = slot.get("v")
                if k_cached is not None and k_cached.shape[2] == hi - lo:
                    if not cloned_k:
                        new_key = key.clone(); cloned_k = True
                    new_key[:, :, lo:hi, :] = k_cached.to(new_key.dtype)
                else:
                    # First time → write-once.
                    slot["k"] = key[:, :, lo:hi, :].detach().clone()
                if v_cached is not None and v_cached.shape[2] == hi - lo:
                    if not cloned_v:
                        new_value = value.clone(); cloned_v = True
                    new_value[:, :, lo:hi, :] = v_cached.to(new_value.dtype)
                else:
                    slot["v"] = value[:, :, lo:hi, :].detach().clone()

        return orig_flex(query, new_key, new_value, block_mask=block_mask, **kwargs)

    cm.rope_apply = patched_rope
    cm.flex_attention = patched_flex
    return orig_rope, orig_flex


def restore_patches(orig_rope, orig_flex):
    cm.rope_apply = orig_rope
    cm.flex_attention = orig_flex


@torch.no_grad()
def run_ar_refresh_appendmatch(
    pipe, *,
    ...
    state = _State()
    orig_rope, orig_flex = install_patches(state)
    ...
    log.info("AR_refresh appendmatch rollout: seed=%d  gen=%d  fifo=%d (global RoPE + write-once post-RoPE K+V cache)",
             seed_frames, num_gen_chunks, fifo_size)

    def _build_position_map(ctx_chunks, current_chunk_id):
        out = []
        pos = 0
    ...
    try:
        for chunk_idx in range(int(num_gen_chunks)):
            ...
            # Set the global RoPE offset — the window's first frame is at
            # global index ctx_global_lo. Same value used for all 4 passes.
            state.global_temporal_offset = ctx_global_lo
            ...
            log.info("[appendmatch] chunk %d/%d committed (id=%d, global_lo=%d) | cached chunks=%s",
                     chunk_idx + 1, num_gen_chunks, current_chunk_id, ctx_global_lo,
                     sorted(state.cache.keys()))
    finally:
        restore_patches(orig_rope, orig_flex)
...
def main():
    ...
    t0 = time.time()
    lat = run_ar_refresh_appendmatch(
        pipe,
    ...
    log.info("ar_refresh_appendmatch latents=%s wall=%.1fs", tuple(lat_cpu.shape), wall)
    ...
    torch.save({
        "meta": {
            ...
            "variant": "ar_refresh_appendmatch",
            "note": "AR_refresh path with global-frame RoPE + write-once post-RoPE K+V cache for ctx chunks; matches append_baseline's caching/RoPE convention",
        },
        "gt": gt_latents,
        "ar_refresh_appendmatch": lat_cpu,
    }, out / "latents.pt")
    log.info("saved %s", out / "latents.pt")

    lat_dev = lat_cpu.to(device=device, dtype=dtype)
    video_np = pipe.decode_latents(lat_dev)
    frames_to_mp4(video_np, str(out / "ar_refresh_appendmatch.mp4"), fps=args.video_fps)
    log.info("wrote %s", out / "ar_refresh_appendmatch.mp4")
    ...
    with (out / "manifest.json").open("w") as fh:
        json.dump({"variant": "ar_refresh_appendmatch", "wall_s": wall, "status": "ok"}, fh, indent=2)
