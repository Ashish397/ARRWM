#!/usr/bin/env python3
"""AR_refresh variant freezing Q AND K together; V refreshes per pass.

Same shape as ``save_latents_ar_refresh_freezeK.py`` but wraps both
``self_attn.q`` and ``self_attn.k``. On pass 0 of each rolling step
both are captured from the model's own forward computation. On
passes 1-3 the cached Q and K are returned in place of self.q/self.k
output. V is left untouched.

Why this is "fairer to the model" than freezing K alone: attention
coefficients ``softmax(Q·K^T)`` are now computed against a Q-K pair
the model legitimately produced together (both from pass-0's input),
so the routing is locked but consistent. V — the values being routed
— refreshes every pass and adapts around the frozen routing.

Saves latents.pt + ar_refresh_freezeQK.mp4 + gt.mp4.
"""
...
class _FreezeMode:
    ...
    def set_capture(self):
        self.capture = True; self.replay = False

    def set_replay(self):
        self.capture = False; self.replay = True


class FreezeWrapper(nn.Module):
    def __init__(self, inner: nn.Linear, mode: _FreezeMode):
        super().__init__()
        self.inner = inner
        self.mode = mode
...
def install_freezeQK_hooks(base_dit) -> tuple[_FreezeMode, list[FreezeWrapper]]:
    mode = _FreezeMode()
    wrappers: list[FreezeWrapper] = []
    n = 0
    for name, module in base_dit.named_modules():
        if hasattr(module, "self_attn"):
            sa = module.self_attn
            if hasattr(sa, "q") and isinstance(sa.q, nn.Linear):
                wq = FreezeWrapper(sa.q, mode); sa.q = wq; wrappers.append(wq); n += 1
            if hasattr(sa, "k") and isinstance(sa.k, nn.Linear):
                wk = FreezeWrapper(sa.k, mode); sa.k = wk; wrappers.append(wk); n += 1
    log.info("freezeQK installed on %d projections (q+k across all blocks)", n)
    return mode, wrappers


def restore_freezeQK_hooks(base_dit, wrappers: list[FreezeWrapper]):
    for name, module in base_dit.named_modules():
        if hasattr(module, "self_attn"):
            sa = module.self_attn
            if hasattr(sa, "q") and isinstance(sa.q, FreezeWrapper):
                sa.q = sa.q.inner
            if hasattr(sa, "k") and isinstance(sa.k, FreezeWrapper):
                sa.k = sa.k.inner
    for w in wrappers:
        w.cached = None
...
def run_ar_refresh_freezeQK(
    ...
    mode, wrappers = install_freezeQK_hooks(base_dit)
    ...
    log.info("AR_refresh freezeQK rollout: seed=%d frames  gen=%d chunks  fifo=%d",
    ...
    try:
        for chunk_idx in range(int(num_gen_chunks)):
            ...
            for w in wrappers: w.cached = None
            ...
            log.info("[freezeQK] chunk %d/%d committed", chunk_idx + 1, num_gen_chunks)
    finally:
        restore_freezeQK_hooks(base_dit, wrappers)
...
def main():
    ...
    log.info("ar_refresh_freezeQK latents=%s wall=%.1fs", tuple(lat_cpu.shape), wall)
    ...
    torch.save({
        "meta": {
            ...
            "variant": "ar_refresh_freezeQK",
            "note": "Q and K both captured on pass 0, replayed on passes 1-3; V refreshed every pass",
        },
        "gt": gt_latents,
        "ar_refresh_freezeQK": lat_cpu,
    }, out / "latents.pt")
    log.info("saved %s", out / "latents.pt")

    lat_dev = lat_cpu.to(device=device, dtype=dtype)
    video_np = pipe.decode_latents(lat_dev)
    frames_to_mp4(video_np, str(out / "ar_refresh_freezeQK.mp4"), fps=args.video_fps)
    log.info("wrote %s", out / "ar_refresh_freezeQK.mp4")
    ...
    with (out / "manifest.json").open("w") as fh:
        json.dump({"variant": "ar_refresh_freezeQK", "wall_s": wall, "status": "ok"}, fh, indent=2)
