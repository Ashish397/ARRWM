...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--action",
        choices=("noop", "straight", "back", "straight_right", "straight_left", "back_right", "back_left", "right", "left"),
        default="noop",
...
def main() -> None:
    ...
    # Select ride directory relative to the caption root so we don't depend on latents
    rel_dir = _select_rel_dir(caption_root, args.ride_name, args.output_ride, args.output_index)
    ...
    # Prepare action sequence based on --action option
    if args.action == "noop":
        pair = (0.0, 0.0)
    elif args.action == "straight":
        pair = (1.0, 0.0)
    elif args.action == "back":
        pair = (-1.0, 0.0)
    elif args.action == "straight_right":
        pair = (0.5, 0.5)
    elif args.action == "straight_left":
        pair = (0.5, -0.5)
    elif args.action == "back_right":
        pair = (-0.5, 0.5)
    elif args.action == "back_left":
        pair = (-0.5, -0.5)
